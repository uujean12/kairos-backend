...
public class AdminController {
    ...
    public ResponseEntity<?> updateOrderStatus(@PathVariable Long id,
        ...
        return orderRepository.findById(id).map(order -> {
            try {
                order.changeStatus(Order.OrderStatus.valueOf(body.get("status")));
                orderRepository.save(order);
                return ResponseEntity.ok(Map.of("message", "상태가 변경되었습니다."));
            } catch (IllegalArgumentException e) {
                return ResponseEntity.<Object>badRequest()
                        .body(Map.of("message", "유효하지 않은 상태입니다."));
            }
    ...
    public ResponseEntity<?> updateUserRole(@PathVariable Long id,
        ...
        return userRepository.findById(id).map(u -> {
            try {
                ...
                return ResponseEntity.<Object>badRequest()
                        .body(Map.of("message", "유효하지 않은 권한입니다."));
            }
        }).orElse(ResponseEntity.notFound().build());
    }
}